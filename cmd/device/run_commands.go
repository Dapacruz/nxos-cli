...
const SESSION_SETUP = "terminal length 0"
...
Examples:
  # Execute the 'show version' and 'show ip arp' commands on rt01.example.com:

    > nxos-cli device run commands --command "show version","show ip arp" rt01.example.com

  # Execute the 'show version' command on rt01.example.com and rt02.example.com, use key based auth, and ignore host key verification:

    > nxos-cli device run commands --command "show version" --key-based-auth --insecure rt01.example.com rt02.example.com`,
...
func init() {
	...
	runCommandsCmd.Flags().IntVarP(&sshTimeout, "ssh-timeout", "S", 30, "SSH connection timeout in seconds")
